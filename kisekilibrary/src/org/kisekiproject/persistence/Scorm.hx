...
class Scorm {
	...
	public function new() {
		...
		if(is_EI_available){
			...
			wrapperFound = ExternalInterface.call("pipwerks.SCORM.isAvailable");
	...
	private function __connect():Bool {
		...
		if(!__connectionActive){
			...
			var eiCall:String = ExternalInterface.call("pipwerks.SCORM.init");
	...
	private function __disconnect():Bool {
		...
		var result:Bool = false;
		if(__connectionActive){
			var eiCall:String = ExternalInterface.call("pipwerks.SCORM.quit");
			result = __stringToBool(eiCall);
			if (result){
	...
	private function __get(parameter:String):String {
		...
		if (__connectionActive){
			...
			returnedValue = ExternalInterface.call("pipwerks.SCORM.get", parameter);
			var errorCode:Int = __getDebugCode();
	...
	private function __save():Bool {
		...
		var result:Bool = false;
		if(__connectionActive){
			var eiCall:String = ExternalInterface.call("pipwerks.SCORM.save");
			result = __stringToBool(eiCall);
			if(!result){
				var errorCode:Int = __getDebugCode();
				var debugInfo:String = __getDebugInfo(errorCode);
	...
	private function __getDebugCode():Int {
		var code:Int = ExternalInterface.call("pipwerks.SCORM.debug.getCode");
		return code;
	}
		
	private function __getDebugInfo(errorCode:Int):String {
		var result:String = ExternalInterface.call("pipwerks.SCORM.debug.getInfo", errorCode);
		return result;
	}
	
	private function __getDiagnosticInfo(errorCode:Int):String {
		var result:String = ExternalInterface.call("pipwerks.SCORM.debug.getDiagnosticInfo", errorCode);
		return result;
	}
	...
	private function __stringToBool(value:Dynamic):Bool {
